...
import { buildTimeSlots, formatPrice, whatsappLink } from "@/lib/format";

const ANY_BARBER = "Любой барбер";
...
const BookingSection = () => {
  ...
  const timeSlots = useMemo(
    () => (branch ? buildTimeSlots(branch.hours.open, branch.hours.close, booking.slotStepMinutes) : []),
    [branch, booking.slotStepMinutes],
  );
  ...
  const handleConfirm = () => {
    ...
    if (isWhatsApp) {
      const lines = [
        `Здравствуйте! Хочу записаться в ${siteConfig.name}.`,
        multiBranch ? `Филиал: ${branch.name} (${branch.address})` : null,
        `Услуга: ${selectedService}`,
        `Барбер: ${selectedBarber}`,
        `Дата: ${dateLabel}`,
        `Время: ${selectedTime}`,
        contactName.trim() ? `Имя: ${contactName.trim()}` : null,
      ].filter(Boolean);
      window.open(whatsappLink(branch.whatsapp, lines.join("\n")), "_blank", "noopener,noreferrer");
      return;
    }
    ...
    toast({
      title: "Вы записаны!",
      description: `${selectedService}, барбер: ${selectedBarber}, ${dateLabel} в ${selectedTime}${
    ...
    <section id="booking" className="py-24 section-padding bg-primary">
      <div className="max-w-[800px] mx-auto">
        ...
        <p className="font-body text-primary-foreground/60 mt-4 text-center">
          Выберите {multiBranch ? "филиал, " : ""}услугу, барбера, дату и время.
        </p>
        ...
        {branch && (
          <div className="mt-10 animate-fade-in">
            <label className="font-body text-primary-foreground/60 text-xs uppercase tracking-widest">Барбер</label>
        ...
        {allSelected && (
          <div className="mt-12 animate-fade-in">
            <label className="font-body text-primary-foreground/60 text-xs uppercase tracking-widest">
              {isWhatsApp ? "Как к вам обращаться" : "Ваши данные"}
            </label>
            <div className={`grid gap-3 mt-3 ${isWhatsApp ? "" : "md:grid-cols-2"}`}>
              <input
                type="text"
                placeholder={isWhatsApp ? "Имя (необязательно)" : "Имя"}
                value={contactName}
